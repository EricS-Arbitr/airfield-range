...
- Per-host directory layout is intentional: the shipper reads the directory name as the `host=` field, so events show up under the **sender** (e.g. `bs-core-rtr`, `bs-ops-fw`) rather than under the collector.
