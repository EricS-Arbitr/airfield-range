...
- **SCADA bus (L2↔L1):** `ff-plc-1`'s built-in **Modbus TCP server** (port 502) exposes the consolidated process image. `control-room-hmi` (FUXA, MTU `/29`) and `fuel-hist` (Telegraf, Fuel-Sim `/24`) read it across the gateway.
...
- Pump permissive = `GROUND_OK ∧ DEADMAN ∧ ¬OVERFILL ∧ (TANK_LVL > LL) ∧ ¬ESD_ACTIVE`.
- Rack overfill → immediate load-valve close + pump stop for that position.
- Tank LL → pump protection lockout.
...
| 8 | `T101_HI_LVL` | T-101 high-level switch |
| 9 | `T101_LO_LVL` | T-101 low-level switch |
| 10 | `ESD_ACTIVE` | Emergency shutdown latched |
...
| 7 | `LR1_METER_LO` | totalizer low word (gal) |
| 8 | `LR1_METER_HI` | totalizer high word (gal) |
| 9 | `LR2_METER_LO` | totalizer low word (gal) |
| 10 | `LR2_METER_HI` | totalizer high word (gal) |
...
> **Totalizers are 32-bit split across two 16-bit registers** (`*_METER_HI`/`*_METER_LO`). Document word order (big-endian / hi-first here) in `group_vars/fuel.yml` so Telegraf and FUXA reassemble them identically.
...
`fuelsim` replay mode reads the timeline, scales `t_offset_s` by `replay.speed` (1.0 = real-time, higher = compressed), and fires each event to the state machine + physics so PLC/HMI/historian/DB all reflect it. **Loopable:** on wrap, offset all timestamps by 48 h for continuous exercises. Determinism (fixed seed) is what makes scenarios repeatable for assessment.

---

## 9. Ansible

**Inventory group** `fuel` → these hosts (the shared `control-room-hmi` also belongs to the power chain). `ansible_host` set to each mgmt IP. `group_vars/fuel.yml` holds the three OT segment CIDRs + gateways, the mgmt block, the full §4 tag map + scaling + totalizer word order, InfluxDB org/bucket, DB name, the tanks/trucks/aircraft/pads seed lists, `replay.speed`, and vault references for all credentials (OpenPLC web, FUXA admin, InfluxDB token, Grafana admin, Postgres roles). Per-host production IP/prefix/gateway live in `host_vars`.

**Roles:**
- `common` — netplan dual-home template (below), NTP client to the range time source, base users/hardening, Wazuh agent.
...
network:
  ...
  ethernets:
    eth0:                      # production OT segment (/29 for MTU & PLC, /24 for Sim)
      ...
      routes:
        - to: default
          via: "{{ prod_gw }}"     # .45.1 MTU · .45.9 Fuel-PLC · .46.16 Fuel-Sim (all = bs-modbus-gateway)
...
Plus a sysctl drop-in setting `net.ipv4.ip_forward=0` on standard hosts. `control-room-hmi` is the exception — a designated bridge with forwarding enabled between its MTU (`172.16.45.3`) and Power-PLC (`172.16.47.9`) NICs only, never onto management. `ansible_host = {{ mgmt_ip }}`. On Ubuntu cloud images the `common` role must neutralize cloud-init networking (`/etc/netplan/50-cloud-init.yaml`) so it doesn't override this dual-home config; netplan itself is native on Ubuntu, so no extra package is needed.
...
| MTU `172.16.45.0/29`, Fuel-Sim `172.16.46.0/24` | `ff-plc-1` :502 | 502/tcp | gw | SCADA reads (HMI/historian→PLC) |
...
- **Replay:** 48 h timeline completes; loop wraps cleanly; identical run with a fixed seed produces identical audit rows.
